...
public class PasswordBasedMacProtection extends MacProtection {
     ...
     *             in case of unsupported algorithm
     */
    public PasswordBasedMacProtection(
            final SharedSecretCredentialContext config)
        ...
        final MessageDigest dig =
                AlgorithmHelper.getMessageDigest(config.getPrf());
        final int iterationCount = config.getIterationCount();
        for (int i = 0; i < iterationCount; i++) {
            calculatingBaseKey = dig.digest(calculatingBaseKey);
            dig.reset();
        }
        final AlgorithmIdentifier macAlgorithm = new AlgorithmIdentifier(
                AlgorithmHelper.getOidForMac(config.getMacAlgorithm()));
        final Mac protectingMac =
                AlgorithmHelper.getMac(macAlgorithm.getAlgorithm().getId());
        protectingMac.init(new SecretKeySpec(calculatingBaseKey,
                protectingMac.getAlgorithm()));
        ...
        final AlgorithmIdentifier protectionAlg = new AlgorithmIdentifier(
                ...
                new PBMParameter(protectionSalt,
                        AlgorithmHelper.findDigestAlgoritm(dig), iterationCount,
                        new AlgorithmIdentifier(AlgorithmHelper
                                .getOidForMac(protectingMac.getAlgorithm()))));
