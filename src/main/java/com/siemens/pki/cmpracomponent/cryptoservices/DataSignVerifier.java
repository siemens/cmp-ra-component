...
public class DataSignVerifier extends TrustCredentialAdapter {
    ...
    private static byte[] verifySignature(
        ...
        for (final SignerInformation signerInfo : signers) {
            ...
            try {
                boolean verified = tryWithAllProviders(p -> signerInfo.verify(
                        new JcaSimpleSignerInfoVerifierBuilder().setProvider(p).build(cert)));
    ...
    public PrivateKey verifySignedKey(final byte[] encodedSignedData)
        ...
        return tryWithAllProviders(p -> new JcaPEMKeyConverter().setProvider(p).getPrivateKey(pki));
    }
}
