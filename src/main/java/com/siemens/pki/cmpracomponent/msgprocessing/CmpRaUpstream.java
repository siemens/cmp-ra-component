...
class CmpRaUpstream implements RaUpstream {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(CmpRaUpstream.class);

    private static final Collection<Integer> supportedMessageTypes =
            new HashSet<>(Arrays.asList(PKIBody.TYPE_INIT_REP,
        ...
        try {
            ...
            if (pesistencyContext.getDelayedDeliveryInProgress()) {
                ...
                case PKIBody.TYPE_POLL_REQ:
                    final PKIMessage delayedResponse =
                            pesistencyContext.getPendingDelayedResponse();
                    if (delayedResponse != null) {
                        ...
                        final PKIHeader delayedRequestHeader =
                                delayedRequest.getHeader();
                        final PKIHeader recHeader = delayedResponse.getHeader();
                        if (!Objects.equals(
                                delayedRequestHeader.getTransactionID(),
                        ...
                        if (!Objects.equals(
                                delayedRequestHeader.getSenderNonce(),
                        ...
                        return PkiMessageGenerator.generateUnprotectMessage(
                                PkiMessageGenerator
                                        .buildRespondingHeaderProvider(in),
                                PkiMessageGenerator.generatePollRep(config
                                        .getRetryAfterTimeInSeconds(certProfile,
            ...
            if (nestedEndpointContext != null) {
                ...
                sentMessage = PkiMessageGenerator.generateAndProtectMessage(
                        ...
                        nestedProtector, new PKIBody(PKIBody.TYPE_NESTED,
            ...
            return PkiMessageGenerator.generateUnprotectMessage(
                    PkiMessageGenerator
                            .buildRespondingHeaderProvider(sentMessage),
                    PkiMessageGenerator.generateResponseBodyWithWaiting(
                            sentMessage.getBody(), INTERFACE_NAME));
    ...
    void gotResponseAtUpstream(final PKIMessage responseMessage)
            throws Exception {
        final PersistencyContext persistencyContext =
                persistencyContextManager.loadPersistencyContext(responseMessage
                        .getHeader().getTransactionID().getOctets());
        if (persistencyContext == null) {
            throw new IllegalStateException(
                    "no related request for known for provided response");
        }
