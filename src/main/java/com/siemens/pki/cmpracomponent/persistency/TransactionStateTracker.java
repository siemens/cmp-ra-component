...
class TransactionStateTracker {
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(TransactionStateTracker.class);
     ...
     *             in case of failed CMP processing
     * @throws IOException
     *             in case of broken ASN.1
     */
    public void trackMessage(final PKIMessage message)
        ...
        if (isError(message)) {
            persistencyContext.setLastTransactionState(
                    LastTransactionState.IN_ERROR_STATE);
            return;
        }
        if (isSecondRequest(message)) {
            if (!Arrays.equals(persistencyContext.getLastSenderNonce(),
        ...
        case INITIAL_STATE:
            if (isGenMessage(message)) {
                persistencyContext.setLastTransactionState(
                        LastTransactionState.GENM_RECEIVED);
                return;
            }
            if (isRevocationRequest(message)) {
                persistencyContext.setLastTransactionState(
                        LastTransactionState.REVOCATION_SENT);
                return;
            }
        ...
        case CERTIFICATE_CONFIRMEND:
            ...
            persistencyContext.setLastTransactionState(
                    LastTransactionState.CONFIRM_CONFIRMED);
            return;
        ...
        case GENM_RECEIVED:
            if (isWaitingIndication(message)) {
                persistencyContext.setLastTransactionState(
                        LastTransactionState.GEN_POLLING);
                return;
            }
            if (!isGenRep(message)) {
                throw new CmpValidationException(INTERFACE_NAME,
            ...
            persistencyContext.setLastTransactionState(
                    LastTransactionState.GENREP_RETURNED);
            return;
        case GEN_POLLING:
            if (isPollRequest(message) || isPollResponse(message)) {
                return;
        ...
        default:
            throw new CmpValidationException(INTERFACE_NAME,
                    ...
                    "transaction in wrong state ("
                            + persistencyContext.getLastTransactionState()
                            + ") for "
                            + MessageDumper.msgAsShortString(message));
        }
    }
    ...
    private void handleCertResponse(final PKIMessage msg)
        ...
        try {
            ...
            dc.getOutputStream()
                    .write(enrolledCertificate.getEncoded(ASN1Encoding.DER));
            ...
            if (!Arrays.equals(persistencyContext.getRequestedPublicKey(),
