...
public class PersistencyContextManager {
    ...
    public static class Asn1ObjectDeserializer<T extends ASN1Object>
        ...
        public T deserialize(final JsonParser p,
            ...
            try {
                return (T) clazz.getMethod("getInstance", Object.class)
                        .invoke(null, binaryValue);
    ...
    public static class Asn1ObjectSerializer
            extends JsonSerializer<ASN1Object> {
        ...
        @Override
        public void serialize(final ASN1Object value,
    ...
    public static class KeySerializer extends JsonSerializer<PrivateKey> {
        ...
        @Override
        public void serialize(final PrivateKey key,
                final JsonGenerator jsonGenerator,
                final SerializerProvider provider) throws IOException {
            if (key == null) {
                jsonGenerator.writeNull();
    ...
    public static class PrivateKeyDeserializer
        ...
        @Override
        public PrivateKey deserialize(final JsonParser p,
                final DeserializationContext ctxt)
                throws IOException, JacksonException {
            final byte[] binaryValue = p.getBinaryValue();
            if (binaryValue == null || binaryValue.length == 0) {
            ...
            try {
                ...
                for (final String keyType : new String[] {"RSA", "EC", "Ed448",
                        "Ed25519"}) {
                    try {
                        return (PrivateKey) c.unwrap(binaryValue, keyType,
                                Cipher.PRIVATE_KEY);
    ...
    private static final Logger LOGGER =
            LoggerFactory.getLogger(PersistencyContextManager.class);

    private static final ObjectMapper objectMapper =
            new ObjectMapper().findAndRegisterModules();

    private static final IvParameterSpec iv =
            new IvParameterSpec("The IV for PrivK".getBytes());

    final SimpleModule simpleModule = new SimpleModule("BCModule",
            new Version(1, 0, 0, null, null, null));

    private final PersistencyInterface wrappedInterface;

    public PersistencyContextManager(
            final PersistencyInterface wrappedInterface) {
        this.wrappedInterface = wrappedInterface;
        final SecretKeySpec secretKey = new SecretKeySpec(
                wrappedInterface.getAesKeyForKeyWrapping(), "AES");
        simpleModule.addSerializer(new Asn1ObjectSerializer());
        simpleModule.addSerializer(new KeySerializer(secretKey));
        simpleModule.addDeserializer(CMPCertificate.class,
                new Asn1ObjectDeserializer<>(CMPCertificate.class));
        simpleModule.addDeserializer(PKIMessage.class,
                new Asn1ObjectDeserializer<>(PKIMessage.class));
        simpleModule.addDeserializer(PrivateKey.class,
                new PrivateKeyDeserializer(secretKey));
    ...
    public PersistencyContext loadCreatePersistencyContext(
            final byte[] transactionId)
            throws DatabindException, IOException, OperatorCreationException {
    ...
    public PersistencyContext loadPersistencyContext(final byte[] transactionId)
            throws StreamReadException, DatabindException, IOException {
        final byte[] serializedPersistency =
                wrappedInterface.getLastSavedMessage(transactionId);
        if (serializedPersistency == null) {
            // transactionId never seen before
            return null;
        }
        // recreate from persistency
        final PersistencyContext ret = objectMapper
                .readValue(serializedPersistency, PersistencyContext.class);
        ret.setContextManager(this);
        return ret;
    }

    void flushPersistencyContext(final PersistencyContext context)
        ...
        wrappedInterface.saveLastMessage(context.getTransactionId(),
                objectMapper.writeValueAsBytes(context));
    }
