...
public class MessageBodyValidator implements ValidatorIF<String> {
     ...
     *            should RaVerified accepted in POPO?
     * @param cmpInterfaceConfig
     *            specific interface (downstream/upstream) configuration
     * @param certProfile
    ...
    public String validate(final PKIMessage message) throws BaseCmpException {
        try {
            final ASN1GeneralizedTime messageTime =
                    message.getHeader().getMessageTime();
            if (messageTime != null) {
                final long diffInMillis =
                        messageTime.getDate().getTime() - new Date().getTime();
                if (!cmpInterfaceConfig
                        .isMessageTimeDeviationAllowed(diffInMillis / 1000L)) {
            ...
            case PKIBody.TYPE_KEY_UPDATE_REQ:
                validateCrmfCertReq(bodyType, (CertReqMessages) content,
                        certProfile, bodyType);
                break;
            ...
            throw new CmpValidationException(interfaceName,
                    ...
                    "internal error in message validation: "
    ...
    private void assertEnrollmentEqual(final int enrollmentType,
            final Object value1, final Object value2, final String errorMsg)
            throws CmpEnrollmentException {
        if (!Objects.equals(value1, value2)) {
            throw new CmpEnrollmentException(enrollmentType, interfaceName,
                    PKIFailureInfo.badDataFormat, errorMsg);
        }

    }

    private void assertEnrollmentValueIsNull(final int enrollmentType,
            final Object value, final int failInfo, final String errorMsg)
            throws CmpEnrollmentException {
    ...
    private void assertEnrollmentValueNotNull(final int enrollmentType,
            final Object value, final int failInfo, final String fieldName)
            throws CmpEnrollmentException {
        if (Objects.isNull(value)) {
            throw new CmpEnrollmentException(enrollmentType, interfaceName,
                    failInfo, "missing '" + fieldName + "'");
        }
    }

    private void assertEqual(final Object value1, final Object value2,
            final String errorMsg) throws CmpValidationException {
        if (!Objects.equals(value1, value2)) {
            throw new CmpValidationException(interfaceName,
                    PKIFailureInfo.badDataFormat, errorMsg);
        }
    ...
    private void assertExactlyOneElementInArray(final Object[] array,
        ...
        if (array.length != 1) {
            throw new CmpValidationException(interfaceName,
                    PKIFailureInfo.badDataFormat,
                    "'" + fieldName + "' must have one element");
        }
        if (array[0] == null) {
            throw new CmpValidationException(interfaceName,
                    PKIFailureInfo.addInfoNotAvailable,
    ...
    private void assertValueIsNull(final Object value, final int failInfo,
            final String errorMsg) throws CmpValidationException {
        if (!Objects.isNull(value)) {
    ...
    private void validateCertConfirm(final CertConfirmContent content)
            throws BaseCmpException {
    ...
    private void validateCertRep(final CertRepMessage content)
        ...
        if (certifiedKeyPair != null) {
            ...
            final CertOrEncCert certOrEncCert =
                    certifiedKeyPair.getCertOrEncCert();
            assertValueNotNull(certOrEncCert, PKIFailureInfo.badDataFormat,
                    "CertOrEncCert");
            assertValueNotNull(certOrEncCert.getCertificate(),
                    PKIFailureInfo.badDataFormat, "Certificate");
        } else {
    ...
    private void validateCrmfCertReq(final int enrollmentType,
            final CertReqMessages content, final String certProfile,
            final int bodyType) throws CmpValidationException {
        ...
        if (versionInTemplate != -1 && versionInTemplate != 2) {
            throw new CmpEnrollmentException(enrollmentType, interfaceName,
                    ...
                    "certTemplate version must be 2");
        }
        assertEnrollmentValueNotNull(enrollmentType, certTemplate.getSubject(),
                PKIFailureInfo.badCertTemplate, "subject in template");
        ...
        if (popo == null) {
            ...
            case ProofOfPossession.TYPE_SIGNING_KEY:
                try {
                    assertEnrollmentValueNotNull(enrollmentType, publicKeyInfo,
                            PKIFailureInfo.badPOP, "publicKey in template");
                    final POPOSigningKey popoSigningKey =
                            (POPOSigningKey) popo.getObject();
                    assertEnrollmentValueIsNull(enrollmentType,
    ...
    private void validateErrorMsg(final ErrorMsgContent content)
            throws CmpValidationException {
        final PKIStatusInfo pkiStatusInfo = content.getPKIStatusInfo();
        validateNegativePkiStatusInfo(pkiStatusInfo);
    }

    private void validateGenMsg(final GenMsgContent content)
            throws BaseCmpException {

        final InfoTypeAndValue[] itav = content.toInfoTypeAndValueArray();
        assertExactlyOneElementInArray(itav, "InfoTypeAndValue");
        assertValueNotNull(itav[0].getInfoType(), PKIFailureInfo.badDataFormat,
                "InfoType");
    }

    private void validateGenRep(final GenRepContent content)
            throws BaseCmpException {
    ...
    private void validateNegativePkiStatusInfo(
            final PKIStatusInfo pkiStatusInfo) throws CmpValidationException {
        switch (pkiStatusInfo.getStatus().intValue()) {
        case PKIStatus.WAITING:
            assertValueIsNull(pkiStatusInfo.getFailInfo(),
                    PKIFailureInfo.badDataFormat,
                    "\"waiting\" and failInfo is set");
            return;
    ...
    private void validateP10CertReq(final CertificationRequest content)
            throws BaseCmpException {
        final PKCS10CertificationRequest p10Request =
                new PKCS10CertificationRequest(content);
        assertValueNotNull(p10Request.getSubject(),
                PKIFailureInfo.badCertTemplate, "Subject");
        try {
            if (!p10Request
    ...
    private void validatePollReq(final PollReqContent content)
            throws BaseCmpException {
    ...
    private void validatePositivePkiStatusInfo(
            final PKIStatusInfo pkiStatusInfo) throws CmpValidationException {
        switch (pkiStatusInfo.getStatus().intValue()) {
        ...
        case PKIStatus.GRANTED_WITH_MODS:
            ...
            return;
        default:
            assertEqual(pkiStatusInfo.getStatus().intValue(),
                    PKIStatus.REJECTION,
                    "status must have have the value \"accepted\" or \"grantedWithMods\"");
        }
    }

    private void validateRevRep(final RevRepContent content)
            throws BaseCmpException {
    ...
    private void validateRevReq(final RevReqContent content)
            throws BaseCmpException {
        ...
        assertValueNotNull(certDetails, PKIFailureInfo.addInfoNotAvailable,
                "certDetails");
        assertValueNotNull(certDetails.getSerialNumber(),
                PKIFailureInfo.addInfoNotAvailable, "SerialNumber");
        assertValueNotNull(certDetails.getIssuer(),
                PKIFailureInfo.addInfoNotAvailable, "Issuer");
        final Extensions crlEntryDetails = revDetails[0].getCrlEntryDetails();
        assertValueNotNull(crlEntryDetails, PKIFailureInfo.addInfoNotAvailable,
                "CrlEntryDetails");
        final Extension reasonCodeExt =
                crlEntryDetails.getExtension(Extension.reasonCode);
        assertValueNotNull(reasonCodeExt, PKIFailureInfo.addInfoNotAvailable,
                "reasonCode");
        final long reasonCode =
                ASN1Enumerated.getInstance(reasonCodeExt.getParsedValue())
                        .getValue().longValue();
        if (reasonCode < 0 || reasonCode > 10) {
            throw new CmpValidationException(interfaceName,
                    PKIFailureInfo.badDataFormat, "reasonCode out of range");
        }
    }
