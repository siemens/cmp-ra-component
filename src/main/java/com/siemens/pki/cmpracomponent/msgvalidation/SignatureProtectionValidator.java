...
class SignatureProtectionValidator implements ValidatorIF<Void> {
    ...
    public Void validate(final PKIMessage message) throws BaseCmpException {
        try {
            final TrustCredentialAdapter trustCredentialAdapter =
                    new TrustCredentialAdapter(config);
            ...
            if (extraCerts != null && extraCerts.length > 0) {
                ...
                final List<X509Certificate> extraCertsAsX509 =
                        CertUtility.asX509Certificates(extraCerts);
            ...
            throw new CmpProcessingException(interfaceName,
                    PKIFailureInfo.notAuthorized, ex.getClass().getSimpleName()
    ...
    private void checkProtectingSignature(final PKIMessage message,
        ...
        if (sender != null) {
            ...
            if (sender.getTagNo() != GeneralName.directoryName || !Objects
                    .equals(sender.getName(), protectionCertSubject)) {
                throw new CmpValidationException(interfaceName,
                        PKIFailureInfo.badMessageCheck, "mismatching sender in "
                                + MessageDumper.msgTypeAsString(message));
            }
        }
    }
