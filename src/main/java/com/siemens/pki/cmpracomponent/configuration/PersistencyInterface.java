...
public interface PersistencyInterface {
    ...
    /**
     * simple local implementation, if providing own implementations overwrite
     * all methods of {@link PersistencyInterface}
     */
    Map<byte[], byte[]> DEFAULT_PERSISTENCY_MAP =
            Collections.synchronizedSortedMap(new TreeMap<>(Arrays::compare));

    byte[] aesKey = CertUtility.generateRandomBytes(16);

    /**
     * clear and forget the last saved message related to a specific
     * transaction.
     *
     * @param transactionId
    ...
    /**
     * provide an AES key to wrap private keys in saved messages. The
     * AES key needs to be stable over the transactions lifetime.
     *
     ...
     * @param transactionId
     *            Id of a specific transaction
     * @return last saved message or <code>null</code> if no message was saved
    ...
    /**
     * save the last PKI request or response related to a specific transaction.
     * Any previously saved message related to the same transaction is dropped.
     *
     * @param transactionId
     *            Id of a specific transaction
     * @param message
     *            message to save
     */
    default void saveLastMessage(final byte[] transactionId,
            final byte[] message) {
