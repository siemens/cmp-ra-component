...
public class PkiMessageGenerator {
     ...
     */
    public static HeaderProvider buildForwardingHeaderProvider(final int pvno,
        ...
        return new HeaderProvider() {
            ...
            public byte[] getRecipNonce() {
                return ifNotNull(header.getRecipNonce(),
            ...
            public byte[] getSenderNonce() {
                return ifNotNull(header.getSenderNonce(),
                        ASN1OctetString::getOctets);
            }
    ...
    public static HeaderProvider buildForwardingHeaderProvider(
            final PKIMessage msg) {
        return buildForwardingHeaderProvider(
                msg.getHeader().getPvno().intValueExact(), msg);
    ...
    public static PKIMessage generateAndProtectMessage(
        ...
        synchronized (protectionProvider) {
            final GeneralName sender = computeDefaultIfNull(
                    protectionProvider.getSender(), headerProvider::getSender);
            final PKIHeaderBuilder headerBuilder = new PKIHeaderBuilder(
                    headerProvider.getPvno(), defaultIfNull(sender, NULL_DN),
            ...
            final CMPCertificate[] generatedExtraCerts =
                    Stream.concat(
                            defaultIfNull(
                                    protectionProvider
                                            .getProtectingExtraCerts(),
                                    Collections.emptyList()).stream(),
                            defaultIfNull(issuingChain, Collections.emptyList())
                                    .stream())
                            .distinct().toArray(CMPCertificate[]::new);
            final DERBitString protection = protectionProvider
                    .getProtectionFor(new ProtectedPart(generatedHeader, body));
            return new PKIMessage(generatedHeader, body, protection,
                    generatedExtraCerts.length == 0 ? null
                            : generatedExtraCerts);
        }
    }
    ...
    public static PKIBody generateErrorBody(final int failInfo,
        ...
        final PKIFreeText statusString =
                ifNotNull(errorDetails, PKIFreeText::new);
     ...
     *             in case of error
     */
    public static PKIMessage generateUnprotectMessage(
