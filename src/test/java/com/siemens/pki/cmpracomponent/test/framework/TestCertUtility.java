...
public class TestCertUtility {
     ...
     * @return all found certificates
     * @throws KeyStoreException if something went wrong
     */
    public static Set<X509Certificate> loadCertificatesFromKeystore(final KeyStore keyStore) throws KeyStoreException {
        ...
        for (final String aktAlias : Collections.list(keyStore.aliases())) {
            ...
            if (aktCert instanceof X509Certificate) {
                ret.add((X509Certificate) aktCert);
                final Certificate[] chain = keyStore.getCertificateChain(aktAlias);
                if (chain != null) {
     ...
     *
     * @param filename name of the key store file
     ...
     */
    public static KeyStore loadTruststoreFromFile(final String filename, final char[] password) throws Exception {
        try {
