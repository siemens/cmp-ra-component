...
public class CmpCaMock {
    ...
    public CmpCaMock(final String enrollmentCredentials,
        ...
        caProtectionProvider = new SignatureBasedProtection(
                new TrustChainAndPrivateKey(protectionCredentials,
                        TestUtils.PASSWORD_AS_CHAR_ARRAY));
    }

    public byte[] processP10CerticateRequest(final byte[] csr,
        ...
        try {
            ...
            return ifNotNull(handleP10CerticateRequest(p10Request),
                    CMPCertificate::getEncoded);
    ...
    private CMPCertificate createCertificate(final X500Name subject,
        ...
        final JcaContentSignerBuilder signerBuilder =
                new JcaContentSignerBuilder(AlgorithmHelper
                        .getSigningAlgNameFromKey(enrollmentCredentials
                                .getPrivateKey())).setProvider(
                                        TestCertUtility.BOUNCY_CASTLE_PROVIDER);

        final CMPCertificate cmpCertificateFromCertificate = TestCertUtility
                .cmpCertificateFromCertificate(new JcaX509CertificateConverter()
                        .setProvider(TestCertUtility.BOUNCY_CASTLE_PROVIDER)
                        .getCertificate(v3CertBldr.build(signerBuilder.build(
    ...
    private PKIMessage handleCrmfCerticateRequest(
        ...
        final List<X509Certificate> issuingChain =
                enrollmentCredentials.getCertificateChain();
        ...
        for (final X509Certificate aktCert : issuingChain) {
            issuingChainForExtraCerts.add(
                    TestCertUtility.cmpCertificateFromCertificate(aktCert));
        }
    ...
    CMPCertificate handleP10CerticateRequest(
        ...
        final List<X509Certificate> issuingChain =
                enrollmentCredentials.getCertificateChain();
        final X509Certificate issuingCert = issuingChain.get(0);
        return createCertificate(certificationRequest.getSubject(),
                certificationRequest.getSubjectPublicKeyInfo(), issuingCert);
    }
