...
public class TestCrWithRAT extends EnrollmentTestcaseBase {
    ...
    private static final String EVIDENCE_FILE = "psa-evidence.cbor";
        ...
        return new ClientContext() {
            ...
            public ClientAttestationContext getAttestationContext() {
                return attestationNonce -> {
                    LOGGER.debug("call attester with nonce " + Hex.toHexString(attestationNonce));
                    try (InputStream in = ConfigFileLoader.getConfigFileAsStream(EVIDENCE_FILE)) {
                        return in.readAllBytes();
                    } catch (final IOException e) {
                        LOGGER.error("error reading " + EVIDENCE_FILE);
                        throw new RuntimeException("error reading " + EVIDENCE_FILE, e);
                    }
                };
            }
    ...
    public void testCr() throws Exception {
        final EnrollmentResult ret = getSignatureBasedCmpClient(
                ...
                .invokeEnrollment();
        assertNotNull(ret);
        System.out.println(ret.getEnrolledCertificate().toString());
    }
}
